...
const COUNTRY_CONFIG: Record<
  string,
  { name: string; stampImage: string; icon: string }
> = {
  japan: { name: "Japan", stampImage: "/stamp/japan.png", icon: "🇯🇵" },
  "south-korea": {
    name: "South Korea",
    stampImage: "/stamp/south-korea.png",
    icon: "🇰🇷",
  },
  ...
  indonesia: {
    name: "Indonesia",
    stampImage: "/stamp/indonesia.png",
    icon: "🇮🇩",
  },
...
export default function PassportPage() {
  ...
  useEffect(() => {
    if (!authLoading && user) {
      info(
        "My Passport",
        `Page 1 of ${isMobile ? totalMobilePages : totalPages}`,
        "🐾"
      );
    }
  }, [authLoading]);
    ...
    if (currentPage > 0) {
      info(
        ...
        `Page ${currentPage + 1} of ${
          isMobile ? totalMobilePages : totalPages
        }`,
        "🐾"
      );
    }
  ...
  const renderCover = () => (
    <div className="absolute inset-0 gradient-warm rounded-r-3xl shadow-2xl flex flex-col items-center justify-center p-4 md:p-8 border-r-4 md:border-r-8 border-accent-dark">
      ...
      >
        <div className="text-5xl md:text-8xl mb-2 md:mb-4 animate-float">
          🐾
        </div>
        ...
        <div className="mt-4 md:mt-8 glass p-3 md:p-4 rounded-xl">
          <div className="text-2xl md:text-3xl font-bold text-gradient">
            {totalStampsCollected}/{totalCountries}
          </div>
          <div className="text-xs text-neutral-dark">Stamps Collected</div>
        </div>
        <div className="text-4xl md:text-6xl opacity-20 absolute bottom-4 md:bottom-8 right-4 md:right-8">
          🐾
        </div>
      </motion.div>
    </div>
  ...
  const renderStampPage = (
    pageStamps: PassportStamp[],
    isRightPage: boolean = false
  ) => (
  ...
  const renderMobilePage = () => {
    const allPages: (
      | { type: "cover" }
      | { type: "stamp"; stamp: PassportStamp; index: number }
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-2 md:p-4 max-w-7xl mx-auto w-full">
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    <div className="grid grid-cols-2 gap-0 h-full">
                      {/* Left Page */}
                      <div
                        className="relative rounded-l-3xl shadow-2xl overflow-hidden"
                      ...
                      {/* Right Page */}
                      <div
                        className="relative rounded-r-3xl shadow-2xl overflow-hidden"
                        style={{ backgroundColor: "#f5f0e8" }}
                      >
                        {/* Passport paper texture */}
                        <div
        ...
        <div className="flex items-center justify-between mt-2 md:mt-4 max-w-md mx-auto px-4 w-full shrink-0">
          ...
          <div className="flex gap-1 flex-wrap justify-center max-w-[180px]">
            {[
              ...Array(isMobile ? Math.min(totalMobilePages, 10) : totalPages),
            ].map((_, index) => (
              <button
                key={index}
