...
export default function FeedbackPage() {
  ...
  const { startExperience, isAudioReady, playQuizResultMusic, playThemeMusic } = useBGM();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        setIsSubmitted(true);
        // Play quiz result music for celebration
        playQuizResultMusic();
        // Refresh user data to get the new stamp
        await checkAuth();
        // Show buttons after a delay so user can view the success animation
        setTimeout(() => {
          setShowButtons(true);
        }, 3500);
      } else {
