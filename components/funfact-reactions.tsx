...
import { useReactions, REACTION_EMOJIS, ReactionType } from "@/lib/reactions-context";
...
export default function FunFactReactions({
  ...
  // Close picker when switching countries
  useEffect(() => {
    setShowPicker(false);
  }, [funfactId]);
  ...
  const handleReaction = async (emoji: ReactionType) => {
    ...
    await addReaction(funfactId, countrySlug, emoji);
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-gray-200 dark:bg-white/10 border border-gray-300 dark:border-white/5"
          >
            <div 
              className="w-4 h-4 rounded-full bg-gray-300 dark:bg-white/20 animate-pulse"
              style={{ animationDelay: `${i * 100}ms` }}
            />
            <div 
              className="w-3 h-3 rounded bg-gray-300 dark:bg-white/15 animate-pulse"
        ...
        {/* Skeleton add button */}
        <div 
          className="w-7 h-7 rounded-full bg-gray-200 dark:bg-white/10 border border-gray-300 dark:border-white/5 animate-pulse" 
    ...
    <div className="relative" ref={pickerRef}>
            ...
            <motion.button
              ...
              className={`group relative flex items-center gap-1 px-2 py-1 rounded-full transition-all duration-200 ${
                userReaction === emoji
                  ...
                  : "bg-gray-100 dark:bg-black/20 border border-gray-200 dark:border-white/10 hover:bg-gray-200 dark:hover:bg-black/30"
              }`}
            >
              <motion.span
                className="text-sm"
              ...
              {!compact && data.users.length > 0 && (
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 z-50">
                  <motion.div
                    ...
                    className="bg-gray-900 dark:bg-white backdrop-blur-xl px-3 py-2 rounded-xl shadow-xl min-w-max"
                  >
                    <div className="flex items-center gap-1.5 mb-1">
                      {data.users.slice(0, 3).map((u) => (
                        <img
                          ...
                          src={u.avatar}
                          alt={u.displayName}
                          className="w-5 h-5 rounded-full border border-white/20 dark:border-gray-200"
                        />
                      ))}
                      {data.users.length > 3 && (
                    ...
                    <p className="text-xs text-white dark:text-gray-900 max-w-[150px] truncate">
                      {data.users
                        .slice(0, 2)
                    ...
                    {/* Arrow */}
                    <div className="absolute top-full left-1/2 -translate-x-1/2 w-0 h-0 border-l-[6px] border-r-[6px] border-t-[6px] border-l-transparent border-r-transparent border-t-gray-900 dark:border-t-white" />
                  </motion.div>
                </div>
              )}
            </motion.button>
      ...
      <AnimatePresence>
        {showPicker && (
          ...
          >
            <div className="bg-white/90 dark:bg-black/80 backdrop-blur-2xl rounded-2xl shadow-2xl border border-gray-200 dark:border-white/10 p-2 overflow-hidden">
              {/* iOS-style liquid glass effect */}
              <div className="absolute inset-0 bg-linear-to-br from-gray-100/50 to-transparent dark:from-white/5 rounded-2xl pointer-events-none" />
